 ...
 * sequential integers (see {@link IndexedGraph#getIndex(Object)}), so we don't
 * need to search for a node in the two lists at all. We can simply use the node
 * index to look up its record in the array (creating it if it is missing). This
 * means that the close list is no longer needed. To know whether a node is open
 * or closed, we use the {@link NodeRecord#category category} of the node
 * record. This makes the search step very fast indeed (in fact, there is no
 * search, and we can go straight to the information we need). Unfortunately, we
 * can't get rid of the open list because we still need to be able to retrieve
 * the element with the lowest cost. However, we use a {@link BinaryHeap} for
 * the open list in order to keep performance as high as possible.
 * 
...
public class IndexedAStarPathFinder<N> implements PathFinder<N> {
	...
	protected void visitChildren(N endNode, Heuristic<N> heuristic) {
		...
		Array<DefaultConnection<N>> connections = graph.getConnections(current.node);

		// Loop through each connection in turn
		for (int i = 0; i < connections.size; i++) {
			if (connections.get(i).valid) {
				if (metrics != null)
					metrics.visitedNodes++;

				DefaultConnection<N> connection = connections.get(i);
	...
	static class NodeRecord<N> extends BinaryHeap.Node {
		...
		DefaultConnection<N> connection;
